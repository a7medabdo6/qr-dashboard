...
import { Header, TypeCard } from './components';
...
const CategoryList = () => {
  ...
  const handlePageClick = event => {
    const newOffset = (event.selected * itemsPerPage) % Types?.length;
    ...
    <Page className={classes.root} title="Category Management List">
      ...
      <div className={classes.results}>
        <Typography color="textSecondary" gutterBottom variant="body2">
          {Types.length} Records found. Page {page + 1} of{' '}
          {Math.ceil(Types.length / itemsPerPage)}
        </Typography>
        {currentItems && currentItems.length > 0 ? (
          currentItems.map(Type => <TypeCard key={Type.id} Type={Type} />)
        ) : (
          <EmptySection />
        )}
      </div>
